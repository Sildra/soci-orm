...
struct json::cx<utils::AuditRows>
{
    ...
    {
        d.SetObject();
        json::serialize_field("INSERTED", f.inserted, d, a);
        json::serialize_field("UPDATED", f.updated, d, a);
        json::serialize_field("MERGED", f.merged, d, a);
        json::serialize_field("IGNORED", f.ignored, d, a);
        json::serialize_field("LOADED", f.loaded, d, a);
        if (f.audit.size() > 0)
            json::serialize_field("AUDIT", f.audit, d, a);
    }
};
